        ...
        <form action="{{ route('trainer.live-classes.store') }}" method="POST" class="space-y-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select name="course_id" class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-[#e3000f]/20 focus:bg-white transition-all text-sm font-bold appearance-none cursor-pointer">
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" name="start_time" required 
                               class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-[#e3000f]/20 focus:bg-white transition-all text-sm font-bold">
                        @error('start_time') <p class="mt-2 text-xs font-bold text-red-500">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        <input type="text" name="duration" required placeholder="e.g. 60 mins" 
                               class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-[#e3000f]/20 focus:bg-white transition-all text-sm font-bold">
                        @error('duration') <p class="mt-2 text-xs font-bold text-red-500">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="url" name="zoom_link" required placeholder="https://zoom.us/j/..." 
                               class="w-full pl-12 pr-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-[#e3000f]/20 focus:bg-white transition-all text-sm font-bold">
                    </div>
                    @error('zoom_link') <p class="mt-2 text-xs font-bold text-red-500">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="pt-6">
                <button type="submit" class="w-full py-5 bg-black text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-[#e3000f] transition-all shadow-xl shadow-slate-900/10 hover:shadow-red-500/20 active:scale-95 transform">
