...
@section('title', 'Course Quiz Management')
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="flex items-center gap-6">
                <div class="w-16 h-16 rounded-[18px] bg-white/5 border border-white/10 flex items-center justify-center backdrop-blur-md shadow-xl">
                    <i class="bi bi-patch-question text-primary text-3xl"></i>
                </div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-[900] tracking-tight uppercase">Quiz <span class="text-primary">Management</span></h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-[24px] border border-border shadow-sm overflow-hidden sticky top-8">
                <div class="p-6 border-b border-border bg-slate-50/50">
                    <h3 class="text-lg font-[900] text-navy uppercase tracking-tight">Add New <span class="text-primary">Question</span></h3>
                    <p class="text-[11px] text-slate-400 font-[700] uppercase tracking-widest mt-1">Populate the exam question bank</p>
                </div>
                <form action="{{ route('admin.courses.quiz.store', $course->id) }}" method="POST" class="p-6 space-y-5">
                    @csrf
                    <div>
                        <label class="block text-[11px] font-[900] text-navy/50 uppercase tracking-[0.2em] mb-2 px-1">The Question</label>
                        <textarea name="question" required placeholder="Enter the question text here..." 
                    ...
                    <div class="space-y-4">
                        <label class="block text-[11px] font-[900] text-navy/50 uppercase tracking-[0.2em] mb-0 px-1 text-center">Options & Correct Answer</label>
                    ...
                    <p class="text-[10px] text-slate-400 font-[700] uppercase tracking-widest text-center py-2 italic border-y border-dashed border-border">
                        Select the letter circle to mark as correct answer.
                    </p>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="flex items-center justify-between mb-2">
                <h2 class="text-xl font-[900] text-navy uppercase tracking-tight">Question Bank ({{ $course->questions->count() }})</h2>
                <div class="h-1 flex-1 mx-6 bg-slate-100 rounded-full"></div>
            </div>

            @forelse($course->questions as $index => $q)
            <div class="bg-white rounded-[24px] border border-border shadow-sm p-6 md:p-8 hover:shadow-xl hover:border-primary/20 transition-all group relative overflow-hidden">
                <div class="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    <form action="{{ route('admin.questions.destroy', $q->id) }}" method="POST" onsubmit="return confirm('Remove this question?')">
                        @csrf
                        @method('DELETE')
            ...
            <div class="bg-white rounded-[24px] border-2 border-dashed border-border p-20 text-center">
                ...
                <h3 class="text-xl font-[900] text-navy uppercase tracking-tight mb-2">Build your Exam</h3>
                <p class="text-slate-500 text-[14px] max-w-sm mx-auto">Start adding multiple choice questions to create the final assessment for this course.</p>
            </div>
            @endforelse
        </div>
    </div>
...
<style>
    ...
    /* Better styling for the checkmark logic in the form */
