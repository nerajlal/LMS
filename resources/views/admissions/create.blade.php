...
<div class="max-w-4xl mx-auto py-10">
    <div class="bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div class="w-full md:w-72 bg-slate-900 p-10 text-white relative overflow-hidden">
            <div class="absolute -top-10 -right-10 w-40 h-40 bg-[#F37021]/20 rounded-full blur-3xl"></div>
            <div class="relative z-10 flex flex-col h-full">
                <div class="w-12 h-12 bg-[#F37021] rounded-2xl flex items-center justify-center text-white text-2xl font-black mb-10 shadow-lg">A</div>
                <h2 class="text-2xl font-black tracking-tight leading-tight mb-6">Complete Your <br><span class="text-[#F37021]">Admission</span></h2>
                <div class="space-y-6 mt-auto">
                    <div class="flex items-center gap-3 text-xs font-bold text-slate-400 tracking-wider">
                        <i class="bi bi-shield-lock-fill text-[#F37021]"></i> SECURE ENROLLMENT
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 p-10 lg:p-14 relative">
            <div class="absolute top-0 right-0 p-8">
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-[#F37021] animate-pulse"></div>
            ...
            <form action="{{ route('admissions.store') }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="md:col-span-2">
                        ...
                        <select name="course_id" required class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-[#F37021]/20 focus:bg-white transition-all text-sm font-bold appearance-none cursor-pointer">
                        ...
                        @error('course_id') <p class="mt-2 text-xs font-bold text-orange-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('full_name') <p class="mt-2 text-xs font-bold text-orange-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" required value="{{ old('phone') }}" 
                               ...
                               class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-[#F37021]/20 focus:bg-white transition-all text-sm font-bold">
                        @error('phone') <p class="mt-2 text-xs font-bold text-orange-600">{{ $message }}</p> @enderror
                    </div>
                ...
                <div class="pt-8">
                    <button type="submit" class="w-full py-5 bg-[#F37021] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#E6631E] transition-all transform hover:-translate-y-1 shadow-xl shadow-orange-500/20 active:scale-95">
